...
from RON_Gauss.ron_gauss_modified import RON_Gauss
...
ratio = float(eps_multiplier)
epsRON = ratio*float(target_eps)
epsGAN = (1-ratio)*float(target_eps)
...
def find_eps(multiplier):
    rdp = compute_rdp(q=sampling_probability,
                  ...
                  steps=steps,
                  orders=orders)
    return(get_privacy_spent(orders, rdp, target_delta=delta)[0]-epsGAN)

noise_multiplier = bisect(find_eps,0.5,3)
...
with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    ...
    G_optim= optimizer.minimize(loss=vector_G_loss, var_list=G_vars)
...
proxy_mnist = input_data.read_data_sets("MNIST_data/", one_hot=True)
proxy_train_set = (proxy_mnist.train.images - 0.5) / 0.5  # normalization; range: -1 ~ 1


# In[ ]:


rongauss = RON_Gauss(algorithm='unsupervised', epsilonMean = 0.3*epsRON, epsilonCov = 0.7*epsRON)
_, _, mu, var = rongauss.generate_dpdata(X=proxy_train_set, dimension=100, reconstruct=False, meanAdjusted=False)
fixed_z_ = np.random.multivariate_normal(mu, var, (25,1,1))
...
# results save folder
root = 'Eps_'+str(target_eps)+'_Trial'+str(trial_num)+'_RON_Private_MNIST_DCGAN_results/'
model = 'RON_Private_MNIST_DCGAN_'
...
for epoch in range(train_epoch):
    ...
    for iter in range(mnist.train.num_examples // batch_size):
        ...
        z_ = np.random.multivariate_normal(mu, var, (batch_size,1,1))
...
np.save(root+'RON_mu.npy',mu)
np.save(root+'RON_var.npy',var)
...
print('total_epsilon = ' + str(epsilon+epsRON) + '\ndelta = ' + str(delta) + 
      '\nnoise multiplier = ' + str(noise_multiplier)
     +'\nRON epsilon = ' + str(epsRON)+
     '\nGAN epsilon = ' + str(epsilon)+
     'Eps ratio = ' + str(ratio))
